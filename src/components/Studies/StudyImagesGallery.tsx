...
interface StudyImagesGalleryProps {
...
const loadPreviewImages = async (
  ...
  const worker = async (): Promise<void> => {
    while (nextIndex < instanceIds.length) {
      ...
      nextIndex += 1;
      try {
        const blob = await getStudyPacsImagePreview(
          studyId,
          instanceIds[index],
        );
...
/**
 * Galería de imágenes PACS de un estudio de la historia (mismo patrón que la
 * galería del editor de informes). Reemplaza al visor Stone en "Ver
 * imágenes": el paciente/médico ve los JPG del PACS sin salir del portal.
 */
export const StudyImagesGallery = ({ studyId }: StudyImagesGalleryProps) => {
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      try {
        const instanceIds = await getStudyPacsImages(studyId);
        if (!active) return;
        if (instanceIds.length === 0) {
          setState({ status: "ready", images: [] });
          return;
        }
        const images = await loadPreviewImages(studyId, instanceIds, context);
  ...
  if (state.images.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center">
        <ImageOff className="h-8 w-8 text-muted-foreground" aria-hidden="true" />
        <p className="text-sm text-muted-foreground">
          Sin imágenes para este estudio.
        </p>
      </div>
    );
  }
      ...
      >
        <DialogContent className="max-w-5xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Vista ampliada del preview del PACS.
            </DialogDescription>
          </DialogHeader>
          {selectedImage?.url && (
            <img
              ...
              alt={
                selectedIndex === null
                  ? "Imagen del estudio"
                  : `Imagen ${selectedIndex + 1} ampliada`
              }
